...
import createStart from "./MemoryRandomizer";
...
const Memory = () => {
  ...
  const renderSpace = () => {
    return spaces.map((space, ind) => {
      return (
        <div key={ind} className="memgrid-box" onClick={() => check(ind)}>
  ...
  return (
    <div>
      ...
      <div className="memgrid">{renderSpace()}</div>
