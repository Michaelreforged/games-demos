...
function TicTacToe() {
  ...
  const renderGrid = () => {
    return Object.entries(space).map((space) => {
      return <div onClick={()=>change(space[0])} className="grid-box" style={{backgroundColor:`${winState.includes(parseInt(space[0]))?'green':''}`}} key={space[0]}>{space[1]}</div>;
    });
  };
  ...
  return (
    <div>
      ...
      {winner? <h2>{winner} is the Winner</h2> :<div className="grid">{renderGrid()}</div>}
